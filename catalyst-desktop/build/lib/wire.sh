...
wire_windows_agents() {
  ...
  esac
  {
    ...
    printf '# Get-Command fails in a shell opened before the build installed uv;\n'
    printf '# ~/.local/bin is where the standalone installer puts it.\n'
    printf '$uv = $null\n'
    printf '$cmd = Get-Command uv -ErrorAction SilentlyContinue\n'
    printf 'if ($cmd) { $uv = $cmd.Source }\n'
    printf 'if (-not $uv) {\n'
    printf '  $fallback = Join-Path $env:USERPROFILE ".local\\bin\\uv.exe"\n'
    printf '  if (Test-Path $fallback) { $uv = $fallback }\n'
    printf '}\n'
    printf 'if (-not $uv) {\n'
    printf '  throw "uv is not installed. Re-run the vault build (it installs uv automatically)."\n'
    printf '}\n'
